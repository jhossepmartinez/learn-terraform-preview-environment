      ...
      <header className="relative z-20 xs:z-50 flex flex-col items-center justify-center bg-white dark:bg-black/50 shadow-mid dark:shadow-highlight h-[120px] xs:h-[164px]">
        <div className="relative flex items-center justify-between max-w-[1080px] w-full">
          ...
          <Link href="/">
            <a className="flex cursor-pointer">
              <Image
                ...
                height={88}
                width={88}
                className="scale-[0.85] xs:scale-100"
              />
            </a>
          </Link>
          <div className="flex items-center justify-end flex-1 px-8 text-black/75">
            <button
              onClick={showAccount}
              className="flex items-center space-x-2 opacity-75 hover:opacity-100 transition duration-500 ease-in-out dark:invert"
            >
              <span className="hidden xs:block text-sm tracking-widest uppercase pt-px">
                {props.isAuthed ? "Account" : "Sign in"}
              </span>
        ...
        <h1 class="font-bold py-4">NUEVO HEADER</h1>
      </header>

      <Account
        accountVisible={props.accountVisible}
        setAccountVisible={props.setAccountVisible}
